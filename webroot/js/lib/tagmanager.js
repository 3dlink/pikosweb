...
(function (jQuery) {
  ...
  jQuery.fn.tagsManager = function (options,tagToManipulate) {
    ...
    var pushTag = function (tag, objToPush, isValid) {
      ...
      var auxTag = tag.split('--');
      var photo = '';
      var idElement = auxTag[0];
      tag = auxTag[1];
      var typeElement = auxTag[2];
      var path = auxTag[3];
      var to = auxTag[4];
      ...
      if (-1 != p) {
        //alreadyInList = true;
        // modificado para que no revice en las listas
        alreadyInList = false;
      }
      ...
      if (alreadyInList) {
        ...
        if (photo) html += '<span class="myTag'+cl+' hphoto" id="' + newTagId + '" >';
        else html += '<span style="margin-top:2px; padding-top:2px; display:inline-block;" class="myTag'+cl+'" id="' + newTagId + '" data-type="'+typeElement+'" data-id="'+idElement+'">';
        if (photo){
          html += '<img src="'+ photo +'">';
        }
        html += '<span data-rel="tooltip" data-placement="top" title="" data-original-title="'+path+'">'+ tag + '&nbsp;&nbsp;</span><a href="#" class="myTagRemover" id="' + newTagRemoveId + '" TagIdToRemove="' + tagId + '" title="Eliminar">' + tagManagerOptions.tagCloseIcon + '</a></span> ';
        // if to la etiqueta va en para, si es cc va para con copia
        if (to == 'to')
          $('#content-receivers').append(html);
        else {
          $('#content-tags-receivers-cc').removeClass('hide');
          $('#content-receivers-cc').append(html);
        }
        ...
        */
        jQuery("#" + newTagRemoveId).on("click", obj, function (e) {
          e.preventDefault();
          $(this).parents('span.myTag').remove();
